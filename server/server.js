...
const FINNHUB_KEY    = "d0eqt0pr01qhup7nkabgd0eqt0pr01qhup7nkac0"; // free public key
const FRED_KEY       = "a9f0a2b3c4d5e6f7a8b9c0d1e2f3a4b5";          // fallback static if FRED fails
...
app.get("/api/calendar", async (req, res) => {
  try {
    ...
    // Fallback static events if Finnhub fails
    res.json({ events: [
...
app.get("/api/realyields", async (req, res) => {
  try {
    // FRED series DFII10 = 10Y Treasury Inflation-Indexed Security
...
// ── GET /api/cot ── Commitment of Traders (static weekly) ───
// COT data is released every Friday by CFTC. We parse the latest.
app.get("/api/cot", async (req, res) => {
  try {
    // CFTC disaggregated futures: gold futures (code 088691)
...
app.get("/api/all", async (req, res) => {
  ...
  try {
    const [priceR,newsR,quoteR,calR,yieldsR,cotR,dxyR] = await Promise.allSettled([
      axios.get(`http://localhost:5000/api/price?interval=${tf}`),
      axios.get(`http://localhost:5000/api/news`),
      axios.get(`http://localhost:5000/api/quote`),
      axios.get(`http://localhost:5000/api/calendar`),
      axios.get(`http://localhost:5000/api/realyields`),
      axios.get(`http://localhost:5000/api/cot`),
      axios.get(`http://localhost:5000/api/dxy-candles?interval=${tf}`),
    ]);
    res.json({
...
app.listen(5000, () => {
  console.log("✅ Gold Signal server v3 running on http://localhost:5000");
